...
#Run analysis script 
# tsp -N <cores_for_tsp> bash run_analysis.sh <chain> <cores> <frames> [cpu|cuda] [gpu_id]
# [cpu|cuda] : Optional, with the default being cpu. Use cuda to run on GPU.
# [gpu_id]   : Optional, with the default being 0. Specify here which GPU to use if using cuda.

# Example run on CPU
tsp -N 4 bash run_analysis.sh  A 4 3 cpu

# Example run on GPU 1
tsp -N 4 bash run_analysis.sh  A 4 3 cuda 1
