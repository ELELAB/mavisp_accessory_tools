...
if __name__ == '__main__':
                    ...
                    try:
                        ...
                        for start, end in value_list:
                            if args.start <= start <= args.end  or args.start <= end <= args.end:
                                all_binding_chains = []
                                for chain in self_chains:
                                    filtered_entries = [entry for entry in complex_partners if not entry.endswith(f', chain_{chain}')]
                                    if filtered_entries:
                                        relevant_chain = filtered_entries[0].split(', ')[2].split('_')[1]
                                        all_binding_chains.append(relevant_chain)
                                all_binding_chains = [item for sublist in all_binding_chains for item in sublist]
                                
                                if args.binding_interface:
                                    binder_dictionary = create_universe_collect_data(structure, self_chains, all_binding_chains, args.distance)
    ...
    output_df = output_df[output_df.residues != "[]"]       
